...
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Dannielle Coe | `reversed` (R-PAY-012) | DANNIELLE COE | u96332808 | All | reversed | not_expected_cancelled | not_expected_cancelled | not_expected_cancelled | Cell-level equal-and-opposite commission pair for `paid_to_date = 2026-01-31`; positive TXN `8245546` and negative TXN `8705401`. Latest authoritative BO `policy_term_date = 2026-01-31`; Feb–Apr re-ledgered to `not_expected_cancelled` via cross-batch supersession. Jan unchanged. |
...
| 3 | Charles Allen | `not_expected_pre_eligibility` (effective date after service month) | Charles Allen | u96954671 | All | not_expected_pre_eligibility | not_expected_cancelled | not_expected_cancelled | not_expected_cancelled | Broker not commission-eligible until 2026-02-01. Jan is the key assertion month. Latest authoritative BO `policy_term_date = 2026-01-31`; Feb–Apr re-ledgered to `not_expected_cancelled` via cross-batch supersession. Jan unchanged. |
...
| 5 | Aaron Barrett | clean `paid` (happy path) | Aaron Barrett | u96603414 | All | paid | not_expected_cancelled | not_expected_cancelled | not_expected_cancelled | Jan paid_amount 24, EDE + active BO + matched commission. Textbook commission-paid scenario. Latest authoritative BO `policy_term_date = 2026-01-31`; Feb–Apr re-ledgered to `not_expected_cancelled` via cross-batch supersession. Jan unchanged. |
| 6 | Adam Wicht | clean `unpaid` (chase path) | Adam Wicht | u97896137 | All | unpaid | not_expected_cancelled | not_expected_cancelled | not_expected_cancelled | Jan has EDE + active BO + `netBucket = +Net`, no commission. Textbook chase scenario. Latest authoritative BO `policy_term_date = 2026-01-31`; Feb–Apr re-ledgered to `not_expected_cancelled` via cross-batch supersession. Jan unchanged. |
...
| 9a | Thomas Mitchell (All-AOR scope) | wrong-AOR scope leakage negative control | Thomas Mitchell | u96757202 | All | not_expected_cancelled | not_expected_cancelled | not_expected_cancelled | not_expected_cancelled | RETIRED — wrong-AOR All-AOR negative control; latest authoritative BO `policy_term_date = 2025-12-31`; Feb–Apr now legitimately cancelled via cross-batch supersession; replacement pending. |
...
| 11 | Chasity Harris | `paid_through_date >= statementMonthEnd` correctly chased (R-INELIG-002 sentinel) | Chasity Harris | u70396792 | All | unpaid | not_expected_cancelled | not_expected_cancelled | not_expected_cancelled | Jan BO has paid_through_date 2026-01-31, no commission. Classifier correctly emits `unpaid` (not suppressed). Proves the R-INELIG-002 correction held. Latest authoritative BO `policy_term_date = 2026-01-31`; Feb–Apr re-ledgered to `not_expected_cancelled` via cross-batch supersession. Jan unchanged. |
