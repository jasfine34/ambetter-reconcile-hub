...
| Dashboard | Jan-Apr 2026 Ambetter, Coverall/Vix/All contexts | `CERTIFIED` for adjusted EBU row membership using normalized-source substitute | `CERTIFIED` for adjusted EBU row membership using MCE Stage 2 anchors | `NEEDS RERUN` | `CERTIFIED` for adjusted EBU row-set overlay consumer path | `CODE-PATH PARITY ONLY` | `NEEDS RERUN` | The Dashboard adjusted EBU counts and member sets exactly match MCE Stage 2 in all 12 contexts, but Est. Missing Commission dollars diverge because MCE defaults blank estimates to $18 while Dashboard sums only stored/remainder estimates. |
...
| Source Coverage / EBU surfaces | Dashboard Jan-Apr 2026 Ambetter, Coverall/Vix/All contexts | `CERTIFIED` for adjusted EBU row membership using normalized-source substitute | `CERTIFIED` for adjusted EBU row membership using MCE Stage 2 anchors | `NEEDS RERUN` | `CERTIFIED` for adjusted EBU row-set overlay consumer path | `CODE-PATH PARITY ONLY` | `NEEDS RERUN` | Source Coverage expected-but-unpaid row sets match Dashboard adjusted EBU row sets, but dollar certification is blocked by the same Dashboard-vs-MCE estimate fallback mismatch. |
