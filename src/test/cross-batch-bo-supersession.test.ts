...
function boRow(over: Partial<NormalizedRecord> & { id: string; batch_id?: string }): NormalizedRecord {
  return {
    id: over.id,
...
function edeRow(over: Partial<NormalizedRecord> & { id: string }): NormalizedRecord {
  return {
    id: over.id,
    source_type: 'EDE',
    staging_status: 'active',
