...
export function reconcile(records: NormalizedRecord[]): { members: ReconciledMember[]; debug: MatchDebugStats } {
  ...
  const edeIsidByEsid = new Map<string, Set<string>>();
  const edeEsidByIsid = new Map<string, Set<string>>();
  for (const r of records) {
    if (r.source_type !== 'EDE') continue;
    if (r.exchange_subscriber_id && r.issuer_subscriber_id) {
      let s = edeIsidByEsid.get(r.exchange_subscriber_id);
      if (!s) { s = new Set(); edeIsidByEsid.set(r.exchange_subscriber_id, s); }
      s.add(r.issuer_subscriber_id);
      let t = edeEsidByIsid.get(r.issuer_subscriber_id);
      if (!t) { t = new Set(); edeEsidByIsid.set(r.issuer_subscriber_id, t); }
      t.add(r.exchange_subscriber_id);
    }
  }
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    // Look up which EDE issuer-sub-ids this esid maps to, and vice versa.
    const edeIsidsForMyEsid = edeIsidByEsid.get(myEsid);
    const edeEsidsForMyIsid = edeEsidByIsid.get(myIsid);
    // If my esid maps to an EDE row whose isid is NOT my isid, AND my isid
    // maps to an EDE row whose esid is NOT my esid, this row cross-links two
    // distinct EDE members.
    const esidPointsToOther =
      edeIsidsForMyEsid && edeIsidsForMyEsid.size > 0 && !edeIsidsForMyEsid.has(myIsid);
    const isidPointsToOther =
      edeEsidsForMyIsid && edeEsidsForMyIsid.size > 0 && !edeEsidsForMyIsid.has(myEsid);
    if (esidPointsToOther && isidPointsToOther) {
      poisonIndices.add(i);
      poisonReasons.set(
        i,
        `Cross-linked IDs: exchange_subscriber_id ${myEsid} belongs to EDE member ${[...(edeIsidsForMyEsid || [])].join(',')} but issuer_subscriber_id ${myIsid} belongs to EDE member with exchange_subscriber_id ${[...(edeEsidsForMyIsid || [])].join(',')}.`
      );
    }
  }
